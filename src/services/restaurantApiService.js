...
export const updateRestaurantStatus = async (restaurantId, status) => {
  try {
    const response = await axios.put(
      `${API_URL}/${restaurantId}/currentStatus`,
...
export const updateCallFrequencyChange = async (
  restaurantId,
  newCallFrequency
) => {
  try {
    const response = await axios.put(
      `${API_URL}/${restaurantId}/callFrequency`,
      { callFrequency: newCallFrequency }
    );
    return response.data;
  } catch (error) {
    handleError(error);
...
export const updateLastCallDate = async (restaurantId, lastCallDate) => {
  try {
    const response = await axios.put(`${API_URL}/${restaurantId}/callDetails`, {
      lastCallDate,
    });
    return response.data;
  } catch (error) {
